                            ...
                            if (!empty($billStudents)):
                                ...
                                <?php foreach ($billStudents as $billStudent): ?>
                                    <tr>    
                                        ...
                                        <td><?php 
                                        if($billStudents[$i]['BillStudent']['nota'] != null){
                                            $nota=$billStudents[$i]['BillStudent']['nota'];
                                        }else{ $nota = '';}
                                        echo $this->Form->input('BillStudent.' . $i . '.nota', array('label' => false, 'onkeypress' => 'return SomenteNumeroVirgula(event)', 'VALUE' => $nota)); 
                                        ?> </td>	
                                    <INPUT TYPE="hidden" NAME="data[BillStudent][<?php echo $i; ?>][discipline_student_id]" VALUE="<?php echo $billStudent['DisciplineStudent']['id']; ?>">
                                    <INPUT TYPE="hidden" NAME="data[BillStudent][<?php echo $i; ?>][id]" VALUE="<?php echo $billStudent['BillStudent']['id']; ?>">
                                    </tr>
                                    <?php $i++;
                                endforeach; ?>
                            <?php endif; ?>
