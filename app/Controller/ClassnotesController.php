 ...
 */
	public function edit($id = null) {
		...
		$this->request->data = $this->Classnote->find('first', $options);
                ...
                $sql = 'SELECT `Frequency`.`id`,`Frequency`.`falta`,`Frequency`.`discipline_student_id`, `DisciplineStudent`.`id`, `DisciplineStudent`.`discipline_groups_id`, `DisciplineStudent`.`students_id`, `DisciplineGroups`.`id`, `DisciplineGroups`.`teacher_id`, `DisciplineGroups`.`discipline_id`, `DisciplineGroups`.`group_id`, `Students`.`id`, `Students`.`nome` FROM `escolabd`.`discipline_students` AS `DisciplineStudent` LEFT JOIN `escolabd`.`discipline_groups` AS `DisciplineGroups` ON (`DisciplineStudent`.`discipline_groups_id` = `DisciplineGroups`.`id`) LEFT JOIN `escolabd`.`students` AS `Students` ON (`DisciplineStudent`.`students_id` = `Students`.`id`) LEFT JOIN `escolabd`.`frequencies` AS `Frequency` ON (`DisciplineStudent`.`id` = `Frequency`.`discipline_student_id` and `Frequency`.`classnote_id` = '.$id.') WHERE `DisciplineGroups`.`id` = '.$iddisciplinegroups;
